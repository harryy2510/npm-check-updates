 ...
 */
function upgradeDependencyDeclaration(
  declaration,
  latestVersion,
  options = {}
) {
  ...
  function chooseVersion(part) {
    return versionUtil.isWildPart(declaredSemver[part])
      ? declaredSemver[part]
      : versionUtil.VERSION_BASE_PARTS.includes(part) && declaredSemver[part]
      ? latestSemver[part]
      : versionUtil.VERSION_ADDED_PARTS.includes(part)
      ? latestSemver[part]
      : undefined;
  }
  ...
  const hasWildCard = versionUtil.WILDCARDS.some((wildcard) =>
    newSemverString.includes(wildcard)
  );
...
function upgradeDependencies(
  ...
  currentDependencies = cint.filterObject(
    currentDependencies,
    (key, value) => value
  );
  ...
  const wildcard =
    getPreferredWildcard(currentDependencies) || DEFAULT_WILDCARD;
  const upgradeDep = (current, latest) =>
    upgradeDependencyDeclaration(current, latest, {
      wildcard,
  ...
  return (
    _(currentDependencies)
      ...
      .pickBy(({ current, currentParsed, latest, latestParsed }) =>
...
function isUpgradeable(current, latest) {
  ...
  return (
    ...
    !isSatisfied(
      latestNormalized,
      range.operator === "<" ? current : version
    ) &&
...
function composeFilter(filterPattern) {
  ...
  // string
    ...
    if (
      filterPattern[0] === "/" &&
      filterPattern[filterPattern.length - 1] === "/"
    ) {
  ...
  // raw RegExp
    ...
    throw new TypeError(
      "Invalid filter. Must be a RegExp, array, or comma-or-space-delimited list."
    );
  }
...
async function getOwnerPerDependency(fromVersion, toVersion, options) {
  ...
  return await Object.keys(toVersion).reduce(async (accum, dep) => {
    ...
    const ownerChanged = await packageManager.packageAuthorChanged(
      dep,
      from,
      to
    );
...
async function upgradePackageDefinitions(currentDependencies, options) {
  ...
  const upgradedDependencies = upgradeDependencies(
    currentDependencies,
    latestVersions,
    {
      removeRange: options.removeRange,
  ...
  const filteredUpgradedDependencies = _.pickBy(
    upgradedDependencies,
    (v, dep) => {
      return (
        !options.jsonUpgraded ||
        !options.minimal ||
        !isSatisfied(latestVersions[dep], currentDependencies[dep])
      );
    }
  );
...
async function upgradePackageData(
  ...
  for (const dependency in newDependencies) {
    if (
      !options.minimal ||
      !isSatisfied(newVersions[dependency], oldDependencies[dependency])
    ) {
      if (options.interactive) {
        const to = versionUtil.colorizeDiff(
      ...
      const expression = `"${dependency}"\\s*:\\s*"${escapeRegexp(
        `${oldDependencies[dependency]}"`
      )}`;
      const regExp = new RegExp(expression, "g");
      newPkgData = newPkgData.replace(
        regExp,
        `"${dependency}": "${newDependencies[dependency]}"`
      );
    }
  }
...
async function getInstalledPackages(options = {}) {
  ...
  const filterFunction = filterAndReject(
    options.filter,
    options.reject,
    options.filterVersion,
    options.rejectVersion
  ...
  return cint.filterObject(
    ...
    (dep, version) =>
      version &&
      !versionUtil.isWildPart(version) &&
      filterFunction(dep, version)
  );
}
...
async function queryVersions(packageMap, options = {}) {
  ...
  async function getPackageVersionProtected(dep) {
    ...
    if (versionUtil.isGithubUrl(packageMap[dep])) {
      versionNew = await packageManagers.gitTags.greatest(
        name,
        version,
        options
      );
      ...
      try {
        ...
        versionNew = npmAlias
          ? versionUtil.createNpmAlias(name, versionNew)
          : versionNew;
      } catch (err) {
        ...
        if (!errorMessage.match(/E404|ENOTFOUND|404 Not Found/i)) {
          ...
          if (
            !process.env.NCU_TESTS &&
            /(Response|network) timeout/i.test(errorMessage)
          ) {
            console.error(
              "\n\n" +
                chalk.red(
  ...
  const versions = await pMap(packageList, getPackageVersionProtected, {
    concurrency: options.concurrency,
  });
...
function getPreferredWildcard(dependencies) {
  ...
  const groups = _.groupBy(Object.values(dependencies), (dep) =>
    versionUtil.WILDCARDS.find((wildcard) => dep && dep.includes(wildcard))
  );
  ...
  const sorted = _.sortBy(
    arrOfGroups,
    (wildcardObject) => -wildcardObject.instances.length
  );
