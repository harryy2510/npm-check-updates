...
function filterOutPrereleaseVersions(versions, pre) {
  return pre
    ? versions
    : versions.filter((version) => !versionUtil.isPre(version));
}
...
module.exports = {
   ...
   */
  list(options = {}) {
    return spawnYarn("list", options, options.cwd ? { cwd: options.cwd } : {})
      .then(async (jsonLines) => {
  ...
  async latest(packageName, currentVersion, options = {}) {
    ...
    if (
      latest &&
      (!versionUtil.isPre(latest.version) || options.pre) &&
      doesSatisfyEnginesNode({ [latest.version]: latest }, options.enginesNode)
        .length
    ) {
      ...
      const versionsSatisfyingNodeEngine = doesSatisfyEnginesNode(
        versions,
        options.enginesNode
      );
      return _.last(
        filterOutPrereleaseVersions(versionsSatisfyingNodeEngine, options.pre)
      );
    }
  },
   ...
   * @returns
   */
  async next(packageName, currentVersion, options = {}) {
    const next = await viewOne(packageName, "dist-tags.next", currentVersion, {
      timeout: options.timeout,
    });

    // if next exists and latest is not a prerelease version, return it
    // if next exists and latest is a prerelease version and --pre is specified, return it
    // if next exists and latest not satisfies min version of engines.node
    if (
      next &&
      (!versionUtil.isPre(next.version) || options.pre) &&
      doesSatisfyEnginesNode({ [next.version]: next }, options.enginesNode)
        .length
    ) {
      return next.version;
      // if next is a prerelease version and --pre is not specified, find the next
      // version that is not a prerelease
    } else {
      return this.latest(packageName, currentVersion, options);
    }
  },
  ...
  newest(packageName, currentVersion, options = {}) {
    return viewManyMemoized(packageName, ["time", "versions"], currentVersion, {
      ...
      .then((result) => {
        const versions = doesSatisfyEnginesNode(
          result.versions,
          options.enginesNode
        );
