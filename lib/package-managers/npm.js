...
function parseJson(result, data) {
  ...
  try {
    ...
    throw new Error(
      `Expected JSON from "${
        data.command
      }". This could be due to npm instability${
        data.packageName
          ? ` or problems with the ${data.packageName} package`
          : ""
      }.\n\n${result}`
    );
  }
 ...
 */
async function packageAuthorChanged(
  packageName,
  currentVersion,
  upgradedVersion
) {
  ...
  if (result.versions) {
    ...
    if (
      current &&
      upgraded &&
      result.versions[current]._npmUser &&
      result.versions[upgraded]._npmUser
    ) {
...
async function viewOne(packageName, field, currentVersion, { timeout } = {}) {
  const result = await viewManyMemoized(packageName, [field], currentVersion, {
    timeout,
  });
...
module.exports = {
  ...
  async next(packageName, currentVersion, options = {}) {
    ...
    // if latest exists and latest is a prerelease version and --pre is specified, return it
    // if latest exists and latest not satisfies min version of engines.node
    if (next && filterPredicate(options)(next)) return next.version;
  ...
  async newest(packageName, currentVersion, options = {}) {
    ...
    const versionsSatisfyingNodeEngine = _.filter(result.versions, (version) =>
      satisfiesNodeEngine(version, options.enginesNode)
    ).map((o) => o.version);
      ...
      (accum, key) =>
        accum.concat(
          TIME_FIELDS.includes(key) ||
            versionsSatisfyingNodeEngine.includes(key)
            ? key
            : []
        ),
      []
